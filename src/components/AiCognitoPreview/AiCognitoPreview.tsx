...
export function AiCognitoPreview() {
  ...
  return <section className="aicognito-preview" aria-label="AiCognito feature walkthrough"><header><strong>AiCognito</strong><span>Live demo</span></header><div className="aicognito-actions" role="group" aria-label="AiCognito capabilities">{capabilities.map((capability) => <button type="button" key={capability.name} aria-pressed={activeName === capability.name} onClick={() => setActiveName(capability.name)}>{capability.name}</button>)}</div><p className="aicognito-description">{active.description}</p><div className="aicognito-screen"><img key={active.name} src={active.image} alt={active.alt} loading="lazy" decoding="async" /></div></section>
}
